...
class Bundle_mod extends CI_Model {
                    ...
                    if ($value != '') {
                        ...
                        elseif (in_array($key, array('logged_account_type', 'logged_current_customer_id', 'logged_account_level'))) {
                            ...
                            if ($value == 'admin') {
                                ...
                                $sub->where("account_type NOT IN ('CUSTOMER','RESELLER') ");
    ...
    public function get_unassigned_data($account_id, $created_by, $filter_data) {
        ...
        try {
            ...
            //bundle_package_id NOT IN(SELECT bundle_package_id FROM bundle_account WHERE account_id='".$account_id."')";
            if (count($filter_data) > 0) {
                foreach ($filter_data as $key => $value) {
                    $sql .= " AND $key ='" . $value . "' ";
                }
            ...
            if ($created_by == 'admin') {
                $sub_sql = "SELECT account_id FROM customers WHERE account_type NOT IN('CUSTOMER','RESELLER')";
                $sql .= " AND created_by IN ($sub_sql)";
            } else {
